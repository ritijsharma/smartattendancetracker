...
const APIAI_ACCESS_TOKEN = process.env.APIAI_ACCESS_TOKEN || 'aaa29646acd949f5804addfb33f87a22';
const APIAI_LANG = process.env.APIAI_LANG || 'en';
const FB_VERIFY_TOKEN = process.env.FB_VERIFY_TOKEN || '123456789';
const FB_PAGE_ACCESS_TOKEN = process.env.FB_PAGE_ACCESS_TOKEN || 'EAAddd4cZAkK0BAK5qNAKWni4CJ7szroeyopLcmHiobOoREmXNZCb2ArRFFl4IkBLlnnnWCVeErKKfjMcKSEZAjm5U0j3qmk7hPDza84vDdLCcv89zMGtxXXPx0TzKefZBan6ZAZCvZAGs6IZAJiMKS19WmVTZBsrnIMjqdYiL6ttyHwZDZD';
    ...
    doApiAiRequest(apiaiRequest, sender) {
        apiaiRequest.on('response', (response) => {
            ...
            if (this.isDefined(response.result) && this.isDefined(response.result.fulfillment)) {
                ...
                if (this.isDefined(responseData) && this.isDefined(responseData.facebook) && !(this.isDefined(action))) {
                    let facebookResponseData = responseData.facebook;
                    this.doDataResponse(sender, facebookResponseData);
                } else if (this.isDefined(responseMessages) && responseMessages.length > 0 && !(this.isDefined(action))) {
                    this.doRichContentResponse(sender, responseMessages);
                } else if (this.isDefined(responseText) && !(this.isDefined(action))) {
                    this.doTextResponse(sender, responseText);
                } else if (this.isDefined(action)) {
                    console.log("===========Entering action===========");
                    this.handleApiAiAction(sender, action, responseMessages, contexts, parameters);
                }
